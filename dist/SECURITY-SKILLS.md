...
Contains 33 security skills compiled at the compact budget tier.
...
| `security-regression-tests` | prevention | medium | * |
...
| `template-injection` | prevention | critical | * |
| `threat-modeling` | prevention | high | * |
...
_Lock every confirmed security fix with a deterministic regression test so CI re-verifies it on every build (no live probing)_
...
_Prevent server-side template injection — user input reaching a template compiler, double-render pipelines, sandbox escapes, and template-driven resource exhaustion_
...
_Review a design or architecture for security flaws — trust boundaries, authorization, data flow, and attack surface — before code is written_
